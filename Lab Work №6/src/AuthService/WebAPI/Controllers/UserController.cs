...
namespace WebAPI.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<ActionResult<User>> Create([FromBody] CreateUserData userData)
        {
            ...
            var createCommand = new Application.Users.Commands.UserFactory(_dbContext);

            try
            {
                var result = createCommand.Create(userData);

                await _dbContext.Users.AddAsync(Persistence.Mappers.UserFactory.CreateModel(result, _dbContext));
